...
1. Compile the sample with **Command+T**, then use **Command+B** after it finishes. Both shortcuts should invoke Compile; repeating either during an active build should not queue another build. Inspect a comment's Original, Proposed replacement, and Changes.
2. Focus the comment controls, use **Shift+S** to skip, and **Shift+A** to accept a valid suggestion without compiling. Confirm that focus stays in the comments so another Shift+A works. Check **Accept** and **Option+Enter** too. Use Undo and confirm that the source and review decision return. Verify uppercase A/S can still be typed normally inside a replacement or note field.
...
8. Open Help and Settings, verify version **1.2.1**, inspect the Changelog, and use **Copy setup details**. Inspect the copied summary for editor/OS/Codex/TeX versions and check status, with no paper text, paths or account data.
