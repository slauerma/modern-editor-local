...
export function reviewSkillConfig(result: unknown, directory: string) {
  ...
  if (!object(entry) || entry.cwd !== directory || !Array.isArray(entry.skills) || entry.skills.length > 1000 || !Array.isArray(entry.errors) || entry.errors.length) throw new CodexPolicyError('The skill inventory is incomplete or cannot be checked.');
  const seen = new Set<string>(); let characters = 0;
  const config = entry.skills.map((skill: unknown) => {
    if (!object(skill) || typeof skill.path !== 'string' || !skill.path || skill.path.length > 10000 || skill.path.includes('\0') || !path.isAbsolute(skill.path) || path.normalize(skill.path) !== skill.path || path.basename(skill.path) !== 'SKILL.md' || typeof skill.enabled !== 'boolean' || seen.has(skill.path)) throw new CodexPolicyError('A discovered skill cannot be disabled reliably.');
    seen.add(skill.path); characters += skill.path.length;
    if (characters > 2_000_000) throw new CodexPolicyError('The skill inventory exceeds the verification limit.');
