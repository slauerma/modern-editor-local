...
function controls(text: string): Control[] {
  ...
  for (let i = 0; i < text.length;) {
    ...
    if (depth === 0) result.push({ name, argument, from, to: group ? argumentStart + group[0].length : i });
...
export function preambleChanges(text: string, proposal: PreambleProposal) {
  ...
  if (additions.some(t => ['newcommand', 'renewcommand', 'providecommand', 'def', 'gdef', 'edef', 'xdef', 'let', 'DeclareMathOperator', 'DeclarePairedDelimiter', 'DeclareRobustCommand', 'NewDocumentCommand', 'RenewDocumentCommand', 'ProvideDocumentCommand'].includes(t.name)))
