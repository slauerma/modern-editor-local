...
type Part = { from: number; to: number; text: string };
...
function parts(text: string, body: { from: number; to: number }): Part[] {
  ...
  for (let i = 1; i < positions.length; i++) result.push({ from: positions[i - 1], to: positions[i], text: text.slice(positions[i - 1], positions[i]) });
  return result;
}
...
function blockRanges(before: string, after: string, a: { from: number; to: number }, b: { from: number; to: number }) {
  ...
  const describe = (p: Part) => ({ kind: /^\s*\\(?:\[|begin\{(?:equation|align|gather|multline))/.test(p.text) ? 'math' : 'prose', words: new Set(p.text.slice(0, 8000).toLowerCase().match(/[\p{L}\p{N}]+/gu) ?? []) });
  for (const run of exactChanges(encodedA, encodedB)) {
    ...
    while (i || j) {
      ...
      if (old?.text !== next?.text) paired.push({ fromA, toA: old?.to ?? fromA, fromB, toB: next?.to ?? fromB });
    }
...
function supported(text: string, source: string, from: number): string | undefined {
  ...
  for (let i = 0; i < text.length; i++) {
    if (text[i] === '%') return 'This block contains a LaTeX source comment. See Text diff for the exact change.';
  ...
  for (const t of controls(text, true)) {
    ...
    } else if (!harmless.has(t.name)) return 'This block contains a command outside the supported comparison subset.';
  }
...
function proseMask(text: string) {
  const mask = new Uint8Array(text.length + 1); let group = 0, option = 0, math = '', environment = 0;
  for (let i = 0; i < text.length;) {
    if (text[i] === '%') { const end = text.indexOf('\n', i); i = end < 0 ? text.length : end; continue; }
    if (text[i] === '\\') {
      const symbol = text[i + 1];
      if (symbol === '[' || symbol === '(') { math = symbol; i += 2; continue; }
      if (symbol === ']' || symbol === ')') { math = ''; i += 2; continue; }
      const command = /^\\([a-zA-Z@]+)/.exec(text.slice(i));
      if (!command) { i += 2; continue; }
      if (!math && !group && !harmless.has(command[1]) && !['label', 'begin', 'end'].includes(command[1])) return new Uint8Array(text.length + 1);
    ...
    if (text[i] === '$') {
      ...
      math = math ? '' : delimiter; i += delimiter.length; continue;
    }
    if (text[i] === '{') { group++; i++; continue; }
    if (text[i] === '}') { group--; i++; continue; }
    if (!math && !group && !environment && text[i] === '[') { option++; i++; continue; }
    if (!math && !group && !environment && text[i] === ']') { option--; i++; continue; }
    if (!group && !option && !math && !environment && !/[&#_^~]/.test(text[i])) mask[i] = 1;
    i++;
  }
...
export function inlineEdits(a: string, b: string) {
  ...
  const left = proseMask(a), right = proseMask(b);
  const ordinary = (text: string, mask: Uint8Array, from: number, to: number) => {
    if (from === to) return !!mask[from] && (from === 0 || !!mask[from - 1]);
...
export function comparisonPlan(before: string, after: string): ComparisonPlan {
  ...
  return { changes: merged.map((c, i) => {
    ...
    const layout: ChangeLayout = omission ? 'omitted' : !oldText.trim() ? 'added' : !newText.trim() ? 'removed' : inline && (repeated || changed < 160 && changed < (oldText.length + newText.length) * .45) ? 'inline' : 'paired';
    return { ...c, id: `change-${i + 1}`, oldText, newText, layout, inline: inline && !omission, paired: !repeated && !omission && !!oldText.trim() && !!newText.trim(), omission, reasons: [] };
  }) };
}
