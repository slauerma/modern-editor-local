...
import { PdfPane, type PdfChangeTarget } from './PdfPane.tsx';
import { comparisonScreenshots } from './comparison-screenshots.ts';

type Props = { input: ChangesInput; visible: boolean; disabled: boolean; proposalCurrent: boolean; previewRequest?: number;
...
type Snapshot = { key: string; proposal?: string; last: ChangesPresentation; values: Partial<Record<ChangesPresentation, ChangesArtifact>> };
...
export function ChangesPdfPane({ input, visible, disabled, proposalCurrent, previewRequest, events, work, onClose, onSource, onText }: Props) {
  const identity = useMemo(() => JSON.stringify([input.projectId, input.before, input.after, input.name, input.engine, input.proposalId, input.selectedPaths]), [input.projectId, input.before, input.after, input.name, input.engine, input.proposalId, input.selectedPaths]);
  ...
  async function show(index: number, value = artifact, expectedKey = key) {
    ...
    try {
      if (value.presentation === 'clean' && !c.newText.trim() && c.layout !== 'omitted') {
        const checked = await window.editor.inspectChanges(input.projectId, value.id);
        if (!mounted.current || id !== request.current || current.current.key !== expectedKey) return;
        if (checked.status !== 'valid') { setInvalidSnapshot(snapshot?.key ?? identity); return; }
        setNoteTarget({ buildId: value.build.id, id: c.id, requestId: id });
        return;
      }
      const location = await window.editor.locateChange(input.projectId, value.id, c.id);
      if (!mounted.current || id !== request.current || current.current.key !== expectedKey) return;
      if (location.kind === 'mapped') setJump({ ...location, requestId: id, persistent: true });
      else setMessage(location.reason);
    } catch (e) { if (mounted.current && id === request.current) setMessage(errorText(e)); }
  }
  async function generate(styleOnly = false) {
    ...
    else { attempted.current = trigger; cursor.current = { ...events }; }
    setWorking(true); setError(''); setJump(null); setNoteTarget(null); request.current++;
    setMessage(styleOnly ? 'Preparing ' + label(presentation).toLowerCase() + ' locally…' : 'Sol is arranging Changes PDF. Generation can take a few minutes.');
    try {
      ...
      active.kind = null; if (!alive()) return;
      const firstShown = styleOnly ? Math.min(selected, Math.max(0, value.changes.length - 1)) : Math.max(0, value.changes.findIndex(c => c.layout !== 'omitted'));
      const next: Snapshot = styleOnly
        ? { ...snapshot!, last: presentation, values: { ...snapshot!.values, [presentation]: value } }
        : { key: identity, proposal: input.proposalId, last: presentation, values: { [presentation]: value } };
      setSaved(next); setSelected(firstShown); if (!styleOnly) { setNoteOpen(false); setInvalidSnapshot(''); } setMessage('');
      if (value.build && next.key === identity) await show(firstShown, value, expected);
  ...
  return <div className="changes-pdf-pane" hidden={!visible} aria-busy={working} data-artifact={artifact?.id} data-presentation={artifact?.presentation}>
    <div className="changes-pdf-actions">
      <div className="comparison-presentation" role="group" aria-label="Changes PDF presentation">
        <button aria-pressed={presentation === 'markup'} disabled={blocked} onClick={() => choosePresentation('markup')}>Revision markup</button>
        <button aria-pressed={presentation === 'clean'} disabled={blocked} onClick={() => choosePresentation('clean')}>Clean paper</button>
      </div>
      <button className={!fresh && !working ? 'primary' : ''} aria-label="Refresh Changes PDF" title="Ask Sol to update the comparison" onClick={() => { setPaused(false); setRefreshRequest(n => n + 1); }} disabled={blocked}>{working ? 'Preparing…' : 'Refresh'}</button>
    </div>
    <div className="changes-pdf-actions changes-navigation-row">
      <div className="change-navigation" role="group" aria-label="Navigate changes">
        ...
        {changes.length ? <select aria-label="Comparison change" value={Math.min(selected, changes.length - 1)} disabled={!fresh} onChange={e => void show(Number(e.target.value))}>
          {changes.map((c, i) => <option key={c.id} value={i}>Change {i + 1} of {changes.length}{c.layout === 'omitted' ? ' · not shown' : ''}</option>)}
        </select> : <span className="change-count">{working ? 'Comparing…' : 'No changes'}</span>}
      ...
      <button className="change-details-toggle text-button" aria-expanded={noteOpen} onClick={() => setNoteOpen(!noteOpen)}>{noteOpen ? 'Hide explanation' : 'Explain change'}</button>
      <button className="text-button" onClick={onText}>Text diff</button>
    </div>
    <div className="changes-agent-status">
      <span role="status">{working ? message : artifact && !fresh ? 'Older comparison · Refresh to update' : paused ? 'Updates paused' : 'Sol · every ' + every + ' accepts and on Save'}</span>
      ...
      <details className="changes-options"><summary>Options</summary><div>
    ...
    {noteOpen && <section className="change-detail" aria-label="Comparison details">
      ...
      {selectedChange && <div className="change-explanation">
        ...
        <div className="change-wording"><div><strong>Before</strong><pre>{selectedChange.oldText || 'No earlier text (addition).'}</pre></div><div><strong>After</strong><pre>{selectedChange.newText || 'Passage deleted.'}</pre></div></div>
        {selectedChange.reasons.map((r, i) => <p key={i}><strong>{r.origin === 'proposal' ? 'Proposal explanation' : 'Recorded acceptance reason'}{r.edited ? ' · edited since suggested' : ''}:</strong> {r.text}</p>)}
        {!selectedChange.reasons.length && <p className="comparison-context">No recorded reason for this change.</p>}
      ...
      {artifact?.notice && <p role="status">{artifact.notice}</p>}
    ...
    <PdfPane changeTarget={fresh ? noteTarget : null} visible={visible && !!artifact?.build} build={artifact?.build ?? null} freshness={fresh ? input.proposalId ? 'Not applied' : label(artifact!.presentation) : 'Older preview'} position={position}
      followComments={false} hideFollow hideClose onFollowChange={() => {}} jump={fresh ? jump : null} onPositionChange={change => setPosition(p => ({ ...p, ...change }))}
      onUserNavigate={() => { request.current++; setJump(null); setNoteTarget(null); }} onClose={onClose}
      changeIds={fresh ? changes.map(c => c.id) : []} onChangeNote={id => { const index = fresh ? changes.findIndex(c => c.id === id) : -1; if (index >= 0) { setSelected(index); setNoteOpen(true); } }} />
  </div>;
}
