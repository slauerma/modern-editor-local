...
export function App() {
  ...
  async function sendChat(projectId: string | null, previewId: string) {
    if (aiBusy || busy || gate.current.locked) throw new Error('Finish or stop the current operation before asking Help me.');
    ...
    try { const answer = await window.editor.sendChat({ projectId }, previewId); if (!gate.current.closing) setStatus('Help me replied · source unchanged'); return answer; }
    finally { setAiBusy(false); done(); }
  }
  ...
  return <><main className="app-shell" aria-busy={locked} {...(locked ? { inert: '' } : {})}>
    <header className={`app-header unified-toolbar ${toolbarCollapsed ? 'collapsed' : ''}`}>
      ...
      <button aria-pressed={helpChatOpen} title="Help me · Command/Ctrl+Shift+H" onClick={() => setHelpChatOpen(v => !v)}>Help me</button>
