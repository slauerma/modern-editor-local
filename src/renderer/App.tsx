        ...
        <aside className="review-rail" aria-label="Source comments" tabIndex={0} onKeyDown={event => {
          ...
          {active ? <article className="comment-card"><h1>{active.title}</h1><p className="explanation">{active.explanation}</p>
            ...
            <div className="comment-actions">
              ...
              {active.replacement !== null && <button disabled={busy || active.decision !== 'open' || active.validity !== 'current'} onClick={() => void acceptWithoutCompile(active.id)} title="Apply the suggestion and listed packages without running LaTeX, then move to the next comment.">Accept without compiling</button>}
            </div>
