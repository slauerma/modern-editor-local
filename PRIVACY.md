...
This policy is currently verified for **Codex CLI 0.153.4** and **0.154.0-alpha.6.2**. Other versions stop before an AI request until their compatibility is tested and the supported-version list is deliberately updated. The [real configuration probe](TESTING.md#real-codex-configuration-check--no-account-request) exercises the same client against harmless inherited MCP servers, without sending a model request or copying account credentials.
