...
test('Shift+A/R/S accept/reject/skip only outside typing fields, without repeat, composition or other modifiers', () => {
  ...
  for (const [key, action] of Object.entries({R:'reject', S:'skip', N:'skip', P:'previous', L:'later', D:'discuss'})) assert.equal(reviewShortcut({ ...event, key }, target(false)), action);
  for (const key of ['A', 'R', 'S', 'N', 'P', 'L', 'D']) {
  ...
  assert.equal(reviewShortcut({ ...event, shiftKey: false, altKey: true, key: 'Backspace' }, target(false)), 'dismiss');
});
